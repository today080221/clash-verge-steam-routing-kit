...
Copy-Item -Force (Join-Path $sourceRoot "sync-clash-verge-steam-script.ps1") (Join-Path $targetRoot "sync-clash-verge-steam-script.ps1")
...
$watcher = Get-CimInstance Win32_Process | Where-Object { $_.CommandLine -match 'sync-clash-verge-steam-script\.ps1' }
if (-not $watcher) {
  Start-Process powershell.exe -ArgumentList '-NoProfile','-ExecutionPolicy','Bypass','-WindowStyle','Hidden','-File', (Join-Path $targetRoot 'sync-clash-verge-steam-script.ps1') | Out-Null
}

Write-Host "Installed Clash Verge Steam routing pack."
